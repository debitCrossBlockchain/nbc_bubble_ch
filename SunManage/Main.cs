...
namespace SunManage
{
    public partial class Main : Form
    {
        ...
        /// 数据节点和链接类型
        /// </summary>
        public static string MTreeName = "不能为空";
        ...
        public void LoadRssTreeList(string url)
        {
            try
            {
                ...
                foreach (XmlNode feed in FeedList)//分别对子元素依次循环操作
                {
                    ...
                    tree.Text = feed.Attributes["title"].Value.ToString();// 设置树型节点的名称
        ...
        public void AddRssFeedSonListTree(XmlNode node, TreeNode PreTN)
        {
            try
            {
                ...
                if (node.HasChildNodes)//测试当前节点有没有子元素
                {
        ...
        private void Main_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!flag)//判断表名是否存在
                {
                    ...
                    table.Columns.Append(column, DataTypeEnum.adWChar, 16);
                ...
                ///显示时间
                /// </summary
                this.MainToolStripStatusLabel.Text = String.Format("系统当前时间：{0}此时选中的设备为：{1}", DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"), MTreeName) + USB.mUSBState;
        ...
        private void MouseDownDelete(object sender, EventArgs e)
        {
            ...
            {
                //CommunicationConnect CCN = new CommunicationConnect();
                DialogResult dr = MessageBox.Show("你确定要删除[" + RssTreeView.SelectedNode.Text + "]节点吗?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void timerMainStatus_Tick(object sender, EventArgs e)
        {
            try
            {
                this.MainToolStripStatusLabel.Text = "系统当前时间：" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss") + "   此时选中的设备为：" + MTreeName + USB.mUSBState;
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        public void OnStatusChanged(string status)
        {
            try
            {
                ...
                this.toolStripStatusLabel1.Text = String.Format("串口的状态为：{0}", status);
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void toolStripMenuItemNew_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.RssTreeView.SelectedNode.Text == "连接列表")
                {
                    #region 增加新设备
                ...
                if (MTreeName != "不能为空" || this.RssTreeView.SelectedNode.Text == "连接列表")
                {
        ...
        private void ToolStripMenuItemDelet_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.RssTreeView.SelectedNode.Text != "连接列表" || MTreeName == "不能为空")
                {
        ...
        private void ToolStripMenuItemQuit_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult dr = MessageBox.Show("你确定是否要退出应用程序！", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void pictureBoxCommunication_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ...
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxCommunication, "通信方式");
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void pictureBoxPrint_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                // p.SetToolTip(this.pictureBoxPrint, "打印");
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void pictureBoxImport_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxImport, "导入");
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void pictureBoxExport_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxExport, "导出");
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void pictureBoxOpenHelpDoc_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxOpenHelpDoc, "打开帮助文档");
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void pictureBoxHelp_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxHelp, "关于");
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile("Exception:" + ex.ToString());
        ...
        private void toolStripMenuItemRest_Click(object sender, EventArgs e)
        {
            try
            {
                if ((MessageBox.Show("你确定重置吗？重置必须是在所有的设备都不在测试状态下进行的！", "警告！", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)) == DialogResult.Yes)
                {
        ...
        private void DiffusionFlowCheckExToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                BasicBubblePoint.BacicBubble = null;
                ...
                WaterImmersionTest.MWaterImmersionTest = null;
                StrengthenTheBubble.MStrengthenTheBubble = null;
                DiffusionFlowCheckEx mDiffusionFlowCheck = new DiffusionFlowCheckEx();
                mDiffusionFlowCheck.TopLevel = false;
                mDiffusionFlowCheck.FormBorderStyle = FormBorderStyle.None;
                mDiffusionFlowCheck.Dock = DockStyle.Fill;
                this.MyPanel.Controls.Clear();
