...
{
    public partial class DiffusionFlowCheckPrinter : Form
    {
        ...
        public DiffusionFlowCheckPrinter()
        {
            InitializeComponent();
        }
        /// <summary>
        ...
        /// <param name="e"></param>
        private void DiffusionFlowCheckPrinter_Load(object sender, EventArgs e)
        {
            try
            {
                this.reportViewer.Reset();
                this.reportViewer.LocalReport.ReportEmbeddedResource = "SunManage.AllCheck.ReportDFCPT.rdlc";
                reportViewer.LocalReport.DataSources.Clear();
                reportViewer.LocalReport.DataSources.Add(new Microsoft.Reporting.WinForms.ReportDataSource("DataSetDFCReport", GetDataSet()));

                reportViewer.LocalReport.DataSources.Add(new Microsoft.Reporting.WinForms.ReportDataSource("DataSetDFCPTReport", ds()));
            ...
            if (!string.IsNullOrEmpty(mTreeName))
            {
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        dr["Test_Value"] = DiffusionFlowCheck.Htest_TestValue.ToString();
                        dr["Test_Result"] = DiffusionFlowCheck.Htest_ALL_Result;
                        dr["Test_Name"] = DiffusionFlowCheck.Htest_Name;
        ...
        private DataTable ds()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= 49; i++)
                {
                    ...
                    if (DiffusionFlowCheck.mCurPT[i].ToString() == "\0" || DiffusionFlowCheck.mCurPT[i].ToString() == "")
                    {
                        DataRow dr = mDataTable.NewRow();
                        dr["DataPressure"] = 0;
                        dr["DataTime"] = i;
                    ...
                    {
                        DataRow dr = mDataTable.NewRow();
                        dr["DataPressure"] = DiffusionFlowCheck.mCurPT[i];
                        dr["DataTime"] = i;
                        mDataTable.Rows.Add(dr);
                    }
        ...
        private void DiffusionFlowCheckPrinter_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
