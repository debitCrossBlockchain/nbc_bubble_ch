...
{
    public partial class StrengthenTheBubble : Form
    {
        int mStatFlagSP;
        static bool mStartTest = false;//为假修改，为真
        public static string Test_AreaSP = (0).ToString("X8");
        public static string Test_Filter_ConfigSP = (0).ToString("X4");
        public static string Test_Filter_numerSP = (0).ToString("X2");
        public static string Test_LIQUConsistenceSP = (0).ToString("X4");


        public static string HTest_Filter_ConfigSP = (0).ToString("X4");
        public static string HTest_Filter_numerSP = (0).ToString("X2");
        public static string HTest_LIQUConsistenceSP = (0).ToString("X4");
        public static string Test_LIQUSP = "";
     ...
     public static   StrengthenTheBubble MStrengthenTheBubble = null;
        Main mTreeName = new Main();
        public StrengthenTheBubble()
        {
            InitializeComponent();
        }
        /// <summary>
        ...
        private void BasicBubblePointPTime(ZedGraphControl zgc)
        {
            ...
            myPane.Title.Text = "增强泡点测试曲线";
            myPane.XAxis.Title.Text = "时间(min)";
            myPane.YAxis.Title.Text = "压力(mbar)";
            ...
            try
            {
                ...
                foreach (CommPort o in CommPort.MCurrent)
                {
                    ...
                    {
                        if ((o.CurrentState == 0x52) && (o.CurrentType == "增强泡点测试"))
                        {
                            labelStrengthenTheBubblePress.Text = o.CurrentPress.ToString();
                            ...
                            if (textBoxTest_startp.Text.ToString() == "\0" || textBoxTest_startp.Text.ToString() == "")
                            {
                                ...
                                foreach (XmlNode node in xml.ChildNodes)
                                {
                                    if (node.Name == "Config")
                                    {
                                        ...
                                        {
                                            if (node1.Name == "StrengthenTheBubble")
                                            {
            ...
            LineItem curve = myPane.AddCurve("压力与时间", list1, Color.Red, SymbolType.None);
            LineItem mcurve = myPane.AddCurve("起测压力", list2, Color.Blue, SymbolType.None);
            LineItem mgcurve = myPane.AddCurve("最小泡点", list3, Color.Green, SymbolType.None);
            ...
            foreach (CommPort oR in CommPort.MResult)
            {
                ...
                {
                    if (oR.CurrentType == "增强泡点测试")
                    {
                        labelValue.Text = oR.Htest_TestValue.ToString();
                        labelDifValue.Text = oR.Htest_DifValue.ToString();
        ...
        private void BasicBubblePointFP(ZedGraphControl zgc)
        {
            ...
            // Set the titles and axis labels
            myPane.Title.Text = "增强泡点测试曲线";
            myPane.XAxis.Title.Text = "压力(mbar)";
            myPane.YAxis.Title.Text = "扩散流(ml/min)";
            myPane.XAxis.Scale.Min = 0;
            ...
            LineItem curve = myPane.AddCurve("起测压力", list, Color.Red, SymbolType.None);
            LineItem mcurve = myPane.AddCurve("最大扩散流", list, Color.Blue, SymbolType.None);
            LineItem mgcurve = myPane.AddCurve("扩散流与压力", list, Color.Green, SymbolType.None);
            LineItem mycurve = myPane.AddCurve("最小泡点", list, Color.Yellow, SymbolType.None); 
        }
       
        private void StrengthenTheBubble_Load(object sender, EventArgs e)
        {
            try
            {
                SunManage.DataBase.DataBase mDataBase = new DataBase.DataBase();
                mDataBase.CreatDB("SBStartTest");
                ...
                string mQuery = "Select [mStatFlagSP] From DeviceConcatenate where [DeviceName]='" + Main.MTreeName + "'";
                ...
                if (reader.Read())
                {
                    mStatFlagSP =Convert.ToInt32( reader[0].ToString());
                }
            ...
            BasicBubblePointPTime(StrengthenTheBubbleZedGraphControlPTime);
            BasicBubblePointFP(StrengthenTheBubbleZedGraphControlFP);
            this.tabControlStrengthenTheBubble.SelectTab(tabPageStrengthenTheBubbleSet);//设置为当前的选项卡
            //实例化xml
            XmlDocument xml = new XmlDocument();
            //读取xml文件
            xml.Load(@"..\..\XmlIni\StartTheTestData.XML");  //xml地址
            ...
            foreach (XmlNode node in xml.ChildNodes)
            {
                            ...
                            foreach (XmlNode node2 in node1.ChildNodes)
                            {
                                switch (node2.Name)
                                {
                                    ...
                                    case "Test_LIQUConsistence":
                                        HTest_LIQUConsistenceSP = node2.InnerText;
                                        break;
                                    case "Test_Filter_Config":
                                        HTest_Filter_ConfigSP = node2.InnerText;
                                        break;
                                    case "Test_Filter_number":
                                        HTest_Filter_numerSP = node2.InnerText;
                                        break;
                                    case "Test_LIQUName":
                                        Test_LIQUSP = node2.InnerText;
                                        break;
                                    default:
                                        break;
                                }
                ...
                this.timerStrengthenTheBubble.Interval = 20000;
                this.timerStrengthenTheBubble.Start();
            }
            }
            catch (Exception ex)
            {
        ...
        private void buttontabPageStrengthenTheBubbleSetTest_Click(object sender, EventArgs e)
        {
            CommPort mComPort = CommPort.Instance;
            string Test_type = (65).ToString("X2");
            ...
            /// <param name="sender"></param>
            /// <param name="e"></param>
            string Test_start = (0).ToString("X4");//起测压力
            if (!string.IsNullOrEmpty(textBoxTest_startp.Text))
            {
                Test_start = (Convert.ToInt64(textBoxTest_startp.Text.Replace(".", ""))).ToString("X4");
            }
            ...
            {
                Test_SetBp = (Convert.ToInt64(textBoxTest_SetBp.Text.Replace(".", ""))).ToString("X4");
            }
            ...
            try
            {
                try
                {
                    ...
                    if (mStartTest == true)
                    {
                        string mQuery = String.Format("INSERT INTO {0}([DeviceName],[Htest_type],[Test_Psernum],[Test_Tsernum],[Test_Fsernum],[Test_filt],[Test_LIQU],[HA_STime],[Test_LIQUType],[Test_LIQUConsistence],[Test_Filter_type],[Test_Filter_Config],[Test_Filter_number],[Test_Filter_Area],[Test_Meme_Aper],[Test_Velocity],[Test_Up_Volm],[Test_startp],[Test_SetBp],[Test_Dif_max]) VALUES ('" + mTreeName + "','" + "增强泡点测试".ToString() + "','" + textBoxTest_Psernum.Text.ToString() + "','" + textBoxTest_Tsernum.Text.ToString() + "','" + textBoxTest_Fsernum.Text.ToString() + "','" + textBoxTest_filt.Text.ToString() + "','" + mLiqu.ToString() + "','" + DTime + "','" + textBoxTest_LIQU.Text.ToString() + "','" + HTest_LIQUConsistenceSP.ToString() + "','" + textBoxTest_Filter_Config.Text.ToString() + "','" + HTest_Filter_ConfigSP.ToString() + "','" + HTest_Filter_numerSP.ToString() + "','" + textBoxTest_Filter_Area.Text.ToString() + "','" + textBoxTest_Meme_Aper.Text.ToString() + "','" + ("").ToString() + "','" + textBoxTest_Up_Volm.Text.ToString() + "','" + textBoxTest_startp.Text.ToString() + "','" + textBoxTest_SetBp.Text.ToString() + "','" + textBoxTest_Dif_max.Text.ToString() + "')", ("SBStartTest").ToString());
                    ...
                    if (mStartTest == false)
                    {
                        ...
                        // string mQuery = String.Format("INSERT INTO {0}([DeviceName],[Htest_type],[Test_Psernum],[Test_Tsernum],[Test_Fsernum],[Test_filt],[Test_LIQU],[HA_STime],[Test_LIQUType],[Test_LIQUConsistence],[Test_Filter_type],[Test_Filter_Config],[Test_Filter_number],[Test_Filter_Area],[Test_Meme_Aper],[Test_Velocity],[Test_Up_Volm],[Test_startp],[Test_SetBp],[Test_Dif_max]) VALUES ('" + mTreeName + "','" + "基本泡点测试".ToString() + "','" + textBoxTest_Psernum.Text.ToString() + "','" + textBoxTest_Tsernum.Text.ToString() + "','" + textBoxTest_Fsernum.Text.ToString() + "','" + textBoxTest_filt.Text.ToString() + "','" + mLiqu.ToString() + "','" + DTime + "','" + textBoxTest_LIQU.Text.ToString() + "','" + HTest_LIQUConsistenceBP.ToString() + "','" + textBoxTest_Filter_Config.Text.ToString() + "','" + HTest_Filter_ConfigBP.ToString() + "','" + HTest_Filter_numerBP.ToString() + "','" + textBoxTest_Filter_Area.Text.ToString() + "','" + textBoxTest_Meme_Aper.Text.ToString() + "','" + mTVelocity.ToString() + "','" + ("").ToString() + "','" + textBoxTest_startp.Text.ToString() + "','" + textBoxTest_SetBp.Text.ToString() + "','" + ("").ToString() + "')", ("BBStartTest").ToString());

                        string mQuery = "update [SBStartTest] set [DeviceName]='" + mTreeName.ToString() + "',[Htest_type]='" + "增强泡点测试".ToString() + "',[Test_Psernum]='" + textBoxTest_Psernum.Text.ToString() + "',[Test_Tsernum]='" + textBoxTest_Tsernum.Text.ToString() + "',[Test_Fsernum]='" + textBoxTest_Fsernum.Text.ToString() + "',[Test_filt]='" + textBoxTest_filt.Text.ToString() + "',[Test_LIQU]='" + mLiqu.ToString() + "',[HA_STime]='" + DTime + "',[Test_LIQUType]='" + textBoxTest_LIQU.Text.ToString() + "',[Test_LIQUConsistence]='" + HTest_LIQUConsistenceSP.ToString() + "',[Test_Filter_type]='" + textBoxTest_Filter_Config.Text.ToString() + "',[Test_Filter_Config]='" + HTest_Filter_ConfigSP.ToString() + "',[Test_Filter_Area]='" + textBoxTest_Filter_Area.Text.ToString() + "',[Test_Meme_Aper]='" + textBoxTest_Meme_Aper.Text.ToString() + "',[Test_Velocity]='" + ("").ToString() + "',[Test_Up_Volm]='" + textBoxTest_Up_Volm.Text.ToString() + "',[Test_startp]='" + textBoxTest_startp.Text.ToString() + "',[Test_SetBp]='" + textBoxTest_SetBp.Text.ToString() + "',[Test_Dif_max]='" + textBoxTest_Dif_max.Text.ToString() + "' where [DeviceName]='" + mTreeName.ToString() + "'";
                ...
                mComPort.SearchDeviceAddress();
                string str = "FF" + "{0}" + "73" + "01" + Test_type + Test_Ps + Test_Ts + Test_Fs + Test_fi + Test_LI + MDataTime + mTest_LIQUType + Test_LIQUConsistenceSP + Test_Filter_ConfigSP + Test_Filter_numerSP + Test_AreaSP + Test_Meme_Aper + Test_Velocity + Test_Up_Volm + Test_start + Test_SetBp + Test_Dif_max + "00";
                str = string.Format(str, CommPort.mDeviceAddress.ToString("X2"));
                byte[] sendData = mComPort.convertstringtobyte(str);
                int sum = 0;
                foreach (int i in sendData)
            ...
            this.tabControlStrengthenTheBubble.SelectTab(tabPageStrengthenTheBubbleState);//设置状态为当前的选项卡
        }

        private void buttonStrengthenTheBubbleLoad_Click(object sender, EventArgs e)
        {
            string mTreeName = Main.MTreeName.ToString();
            ...
            if (!string.IsNullOrEmpty(mTreeName))
            {
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        HTest_Filter_ConfigSP = reader[11].ToString();
                        HTest_Filter_numerSP = reader[12].ToString();
                        HTest_LIQUConsistenceSP = reader[9].ToString();
                        ...
                        {
                            Test_Filter_ConfigSP = (0).ToString("X4");
                        }
                        else
                        {
                            Test_Filter_ConfigSP = (Convert.ToInt64(reader[11].ToString())).ToString("X4");
                        }
                        if (string.IsNullOrEmpty(reader[12].ToString()))
                        {
                            Test_Filter_numerSP = (0).ToString("X2");
                        }
                        else
                        {
                            Test_Filter_numerSP = (Convert.ToInt64(reader[12].ToString())).ToString("X4");
                        }
                        if (string.IsNullOrEmpty(reader[9].ToString()))
                        {
                            Test_LIQUConsistenceSP = (0).ToString("X4");
                        }
                        else
                        {
                            Test_LIQUConsistenceSP = (Convert.ToInt64(reader[9].ToString())).ToString("X4");
                        }
        ...
        private void StrengthenTheBubble_FormClosed(object sender, FormClosedEventArgs e)
        {
            MStrengthenTheBubble = null;
        }
        /// <summary>
        /// 定时器增强泡点测试
        /// 显示当前参数
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void timerStrengthenTheBubble_Tick(object sender, EventArgs e)
        {
            CommPort mComPort = CommPort.Instance;
            if ((mComPort.DeviceAddress1 == CommPort.mDeviceAddress) && (mComPort.StateTest_type1 == "增强泡点测试"))
            {
                labelStrengthenTheBubblePress.Text = mComPort.Test_press1;
            }
        }
        ...
        {
            DialogResult dr = MessageBox.Show("你确定要保存增强泡点测试结果吗?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            string m1 = "";
            string m2 = "";
            string m3 = "";
            string m4 = "";
            ...
            if (DialogResult.Yes == dr)
            {
                ...
                if (!string.IsNullOrEmpty(mTreeName))
                {
                    ...
                    string mQuery = "Select TestHisData as 序号,Test_Psernum as 生产序号,HA_STime as 测试时间,Htest_type as 测试类型 From {0}";
        ...
        private void pictureBoxPrint_Click(object sender, EventArgs e)
        {
            try
            {
            StrengthenTheBubblePrinter mStrengthenTheBubblePrinter = new StrengthenTheBubblePrinter();
            mStrengthenTheBubblePrinter.ShowDialog();
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile ("Exception:" + ex.ToString());
        ...
        private void pictureBoxChart_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.tabControlStrengthenTheBubble.SelectTab(tabPageStrengthenTheBubbleChart);//设置为当前的选项卡
            }
            catch (Exception ex)
            {
                LogClass.WriteLogFile ("Exception:" + ex.ToString());
        ...
        private void pictureBoxChart_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ...
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxChart, "查看增强泡点测试曲线");
        ...
        private void pictureBoxSave_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ...
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxSave, "保存增强泡点测试结果");
        ...
        private void pictureBoxPrint_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ...
                ToolTip p = new ToolTip();
                p.ShowAlways = true;
                p.SetToolTip(this.pictureBoxPrint, "打印增强泡点测试结果");
        ...
        private void textBoxTest_Filter_Config_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                textBoxTest_Filter_Area.Text = Barrel.Test_filter_Area;
        ...
        private void textBoxTest_Filter_Config_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case "桶式过滤器":
                        Test_Filter_ConfigSP = Barrel.Test_filter_Hight;
                        Test_Filter_numerSP = Barrel.Test_filter_Num;
                        break;
                    case "平板过滤器":
                        Test_Filter_ConfigSP = Plate.Test_Filter_Diameter;
        ...
        private void textBoxTest_Filter_Area_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Test_AreaSP = (Convert.ToInt64(((textBoxTest_Filter_Area.Text).Replace(".", "")))).ToString("X8");
                }
                else
                {
                    Test_AreaSP = (0).ToString("X8");
                }
        ...
        private void textBoxTest_LIQU_TextChanged(object sender, EventArgs e)
        {
            try
            {
                Test_LIQUConsistenceSP = Concentration.mTest_LIQUConsistence;
                if (textBoxTest_LIQU.Text == "其它")
                {
                    Test_LIQUSP = Fluid.Test_Filter_LiquidName;
                }
                else
                {
                    Test_LIQUSP = textBoxTest_LIQU.Text;
                }
            }
            catch (Exception ex)
            {
        ...
        private void pictureBoxstart_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mStatFlagSP == 1)
                {
                    ...
                    string mQuery = "update DeviceConcatenate set [mStatFlagSP]='" + (0).ToString() + "' where [DeviceName]='" + (Main.MTreeName).ToString() + "'";
                ...
                else
                {
                    ...
                    string mQuery = "update DeviceConcatenate set [mStatFlagSP]='" + (1).ToString() + "' where [DeviceName]='" + (Main.MTreeName).ToString() + "'"; ;
